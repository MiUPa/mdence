...
export function Demo() {
  const [isMDence, setIsMDence] = useState(true);
  ...
  // When switching to MDence mode, only trigger update if content actually changed
  const handleSwitchToMDence = useCallback(() => {
    if (!isMDence && rawModeEdited.current && markdown !== markdownWhenSwitchedToRaw.current) {
        ...
        <div className="mx-auto mt-16 max-w-5xl">
            ...
            <div className="relative inline-flex items-center rounded-full bg-slate-200 p-1 dark:bg-slate-700">
              ...
              <div
                className={`absolute h-8 w-28 rounded-full bg-white shadow-md transition-transform duration-300 ease-out dark:bg-slate-900 ${
                  isMDence ? 'translate-x-0' : 'translate-x-28'
                }`}
              />

              <button
                onClick={handleSwitchToMDence}
          ...
          <div className="overflow-hidden rounded-2xl bg-slate-900 shadow-2xl ring-1 ring-white/10">
            ...
            <div className="flex items-center gap-2 border-b border-slate-700 bg-slate-800 px-4 py-3">
              ...
              <div className="flex-1 text-center">
                <span className="text-sm text-slate-400">README.md — MDence</span>
              </div>
            </div>

            {/* Editor content with crossfade animation */}
            <div className="relative">
              {/* MDence editor - always in flow to set container height */}
              <div
                className={`min-h-[400px] p-8 font-sans text-slate-100 transition-opacity duration-300 ${
                  isMDence ? 'opacity-100' : 'pointer-events-none opacity-0'
                }`}
              >
                <DemoEditor
              ...
              <div
                className={`absolute inset-0 flex transition-opacity duration-300 ${
                  isMDence ? 'pointer-events-none opacity-0' : 'opacity-100'
                }`}
              >
                <textarea
          ...
          <p className="mt-4 text-center text-sm text-slate-500 dark:text-slate-400">
            {isMDence ? (
              ...
              <>
                Edit the raw Markdown here. Changes sync when you switch back to MDence view.
              </>
            )}
          </p>
        </div>

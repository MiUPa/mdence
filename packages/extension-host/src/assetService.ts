...
export class AssetService {
  ...
  private validateAssetsFolder(folder: string): string {
    ...
    if (normalized.includes('..')) {
      console.warn('MDence: Assets folder contains "..", using default');
      return 'assets';
    }

    // Reject absolute paths
    if (path.isAbsolute(normalized)) {
      console.warn('MDence: Assets folder is absolute path, using default');
      return 'assets';
    }

    // Reject if starts with slash or backslash
    if (normalized.startsWith('/') || normalized.startsWith('\\')) {
      console.warn('MDence: Assets folder starts with separator, using default');
      return 'assets';
    }

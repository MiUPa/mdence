...
export class MDenceEditorProvider implements vscode.CustomTextEditorProvider {
  public static readonly viewType = 'mdence.editor';
  ...
  public static register(context: vscode.ExtensionContext): vscode.Disposable {
    const provider = new MDenceEditorProvider(context);
    const providerRegistration = vscode.window.registerCustomEditorProvider(
      MDenceEditorProvider.viewType,
      provider,
      {
        webviewOptions: {
  ...
  async resolveCustomTextEditor(
    ...
    console.log('MDence: resolveCustomTextEditor called for', document.uri.toString());
    ...
    webviewPanel.webview.options = {
      ...MDenceEditorProvider.webviewOptions,
      localResourceRoots,
    };
    ...
    const messageHandler = webviewPanel.webview.onDidReceiveMessage(
      async (rawMessage: unknown) => {
        ...
        if (!parseResult.success) {
          console.error('MDence: Invalid message from webview:', parseResult.error.message);
          return;
        }

        const message = parseResult.data;
        console.log('MDence: Received validated message from webview:', message.type);
    ...
    const configChangeSubscription = vscode.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration('mdence')) {
  ...
  private getHtmlForWebview(webview: vscode.Webview): string {
    ...
    console.log('MDence: Extension URI:', this.context.extensionUri.toString());
    console.log('MDence: Script URI:', scriptUri.toString());
    console.log('MDence: Style URI:', styleUri.toString());
    console.log('MDence: CSP:', csp);
...
<body>
  <div id="root">
    <div style="padding: 20px; color: #888;">Loading MDence editor...</div>
  </div>
  <script nonce="${nonce}">
    console.log('MDence inline script running');
    window.onerror = function(msg, url, line, col, error) {
      console.error('MDence error:', msg, url, line, col, error);

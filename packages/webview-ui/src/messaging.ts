...
export function requestInit(): void {
  console.log('MDence: Requesting init from host');
...
export function addMessageHandler(handler: MessageHandler): () => void {
  ...
  if (messageHandlers.size >= MAX_HANDLERS) {
    console.error('MDence: Max message handlers reached. This indicates a memory leak - handlers are not being cleaned up properly.');
...
function initMessageListener(): void {
  console.log('MDence: Setting up message listener');
  window.addEventListener('message', (event) => {
    console.log('MDence: Received message from host:', event.data);

    // SECURITY: Runtime validation of incoming messages
    const message = validateHostToUIMessage(event.data);
    if (!message) {
      console.warn('MDence: Ignoring invalid message from host');
      return;
    }
    ...
    messageHandlers.forEach((handler) => {
      try {
        ...
        console.error('MDence: Error in message handler:', error);
      }
    });
  });
}

...
// Settings schema
export const MDenceSettingsSchema = z.object({
...
export type MDenceSettings = z.infer<typeof MDenceSettingsSchema>;
...
export function validateUIToHostMessage(data: unknown): UIToHostMessage | null {
  ...
  if (!result.success) {
    console.error('MDence: Invalid UI→Host message:', result.error.message);
    return null;
  }
  return result.data;
}
...
export function validateHostToUIMessage(data: unknown): HostToUIMessage | null {
  ...
  if (!result.success) {
    console.error('MDence: Invalid Host→UI message:', result.error.message);
    return null;
  }
  return result.data;
}

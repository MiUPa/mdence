...
async function main() {
  ...
  const vsixPath = path.join(EXTENSION_DIR, "mdence.vsix");
  if (!fs.existsSync(vsixPath)) {
    console.error(chalk.red("\n  ✗ mdence.vsix not found after packaging\n"));
    process.exit(1);
  }
  ...
  await runStep("Publishing to VS Code Marketplace", () => {
    exec(`npx vsce publish --packagePath mdence.vsix`, { cwd: EXTENSION_DIR });
  });
  ...
  await runStep("Publishing to Open VSX", () => {
    exec(`npx ovsx publish mdence.vsix -p ${ovsxToken}`, {
      cwd: EXTENSION_DIR,
    });
  });
  ...
  console.log(
    chalk.gray("  VS Code:      ") +
      chalk.cyan(
        "https://marketplace.visualstudio.com/items?itemName=miupa.mdence"
      )
  );
  console.log(
    chalk.gray("  Open VSX:     ") +
      chalk.cyan("https://open-vsx.org/extension/miupa/mdence")
  );
  console.log();
}
